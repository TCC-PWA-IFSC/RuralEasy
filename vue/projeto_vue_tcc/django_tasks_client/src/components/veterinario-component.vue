<template>
<div class="main">
    <div class="container">
      <h2>Cadastro de Veterinário</h2>
      <form @submit.prevent="submitForm">
        <div>
          <label for="nome">Nome:</label>
          <input type="text" id="nome" v-model="veterinario.nome" required>
        </div>
        <div>
          <label for="telefone">Telefone:</label>
          <input type="text" id="telefone" v-model="veterinario.telefone" required>
        </div>
        <div>
          <label for="email">Email:</label>
          <input type="text" id="email" v-model="veterinario.email" required>
        </div>
        <div>
          <label for="crmv">CRMV:</label>
          <input type="text" id="crmv" v-model="veterinario.crmv" required>
        </div>
        <button type="submit">Salvar</button>
        <!--<button v-if="isEdit" @click.prevent="deleteVeterinario">Excluir</button>-->
        <button v-if="isEdit" @click.prevent="cancelEdit">Cancelar</button>
      </form>
    </div>

    <div class="table">
      <h1>Lista de Veterinários</h1>
      <table>
        <thead>
          <tr>
            <th>Nome</th>
            <th>Telefone</th>
            <th>Email</th>
            <th>CRMV</th>
            <th>Ações</th>
          </tr>
        </thead>
        <tbody>
          <tr v-for="veterinario in veterinarios" :key="veterinario.id">
            <td>{{ veterinario.nome }}</td>
            <td>{{ veterinario.telefone }}</td>
            <td>{{ veterinario.email }}</td>
            <td>{{ veterinario.crmv }}</td>
            <td>
              <button @click.prevent="editVeterinario(veterinario)">Editar</button>
              <button @click.prevent="confirmDelete(veterinario)">Excluir</button>
            </td>
          </tr>
        </tbody>
      </table>
    </div>

    <div class="botaoConfirmaDelete" v-if="showDeleteModal">
      <h3>Tem certeza que deseja excluir o veterinário {{ veterinario.nome }}?</h3>
      <button @click.prevent="deleteVeterinarioConfirmed">Sim</button>
      <button @click.prevent="cancelDelete">Não</button>
    </div>
  </div>
  </template>
  
  <script>
  import axios from 'axios'
  
  axios.defaults.xsrfCookieName = 'csrftoken';
  axios.defaults.xsrfHeaderName = 'X-CSRFToken';
  
  export default {
    data() {
      return {
        veterinario: {
          nome: '',
          telefone: '',
          email: '',
          crmv: ''
        },
        veterinarios: [],
        isEdit: false,
        showDeleteModal: false,
        veterinarioToDelete: null
      }
    },
    methods: {
  
       //editar 
  editVeterinario(veterinario) {
    this.isEdit = true;
    this.veterinario = { ...veterinario };
  },
      
  //adicionar
  submitForm() {
    if (this.isEdit) {
      console.log("Aqui", this.veterinario.id)
      axios.put(`http://localhost:8000/veterinarios/editar/${this.veterinario.id}/`, this.veterinario)
        .then(response => {
          this.veterinarios = response.data;
          this.isEdit = false; // Redefinir o estado de edição pra não dar ruim
          this.veterinario = {
            nome: '',
            telefone: '',
            email: '',
            crmv: ''
          };
        })
        .catch(error => {
          console.log(error);
        });
    } else {
      console.log('Método POST')
      axios.post('http://localhost:8000/veterinarios/', this.veterinario)
        .then(response => {
          this.veterinarios = response.data;
          this.veterinario = {
            nome: '',
            telefone: '',
            email: '',
            crmv: ''
          };
        })
        .catch(error => {
          console.log(error);
        });
    }
    setTimeout(function() {
      location.reload();
    }, 2000);
  },
  
      deleteVeterinario() {
        axios.delete(`http://localhost:8000/veterinarios/deleteveterinario/${this.veterinario.id}/`)
          .then(response => {
            this.veterinarios = response.data
          })
          .catch(error => {
            console.log(error)
          })
      },
      confirmDelete(veterinario) {
        if (window.confirm("Você tem certeza de que deseja excluir esta propriedade?")) {
          this.veterinarioToDelete = veterinario;
          this.deleteVeterinarioConfirmed();
        }
      },
      deleteVeterinarioConfirmed() {
        axios.delete(`http://localhost:8000/veterinarios/deleteveterinario/${this.veterinarioToDelete.id}/`)
          .then(response => {
            this.veterinarios = response.data
            this.showDeleteModal = false
          })
          .catch(error => {
            console.log(error)
          })
          setTimeout(function() {
      location.reload();
      }, 2000);
      },
      cancelDelete() {
        this.showDeleteModal = false
      },
      cancelEdit() {
        window.location.href = '/veterinarios'
      },
      fetchVeterinario(id) {
        axios.get(`http://localhost:8000/veterinarios/${id}/`)
          .then(response => {
            this.veterinario = response.data
          })
          .catch(error => {
            console.log(error)
          })
      }
    },
    
    mounted() {
      axios.get('http://localhost:8000/veterinarios/')
        .then(response => {
          this.veterinarios = response.data
        })
        .catch(error => {
          console.log(error)
        })
      /*axios.get('http://localhost:8000/racas/')
        .then(response => {
          this.racas = response.data
        })
        .catch(error => {
          console.log(error)
        })*/
    }
  
  }
  </script>


<style scoped>

@import url('https://fonts.googleapis.com/css2?family=Poppins:wght@400;500;600&display=swap');

/* Base para aplicar a fonte em todo o componente */
.container {
    font-family: 'Poppins', sans-serif;
    text-align: center;
}

button {
    padding: 10px 20px;
    margin-right: 10px;
    border: none;
    border-radius: 5px;
    cursor: pointer;
    transition: all 0.3s;
    background-color: #28a745; /* verde */
    color: #FFF;
    font-weight: 500;
    box-shadow: 0 2px 8px rgba(40, 167, 69, 0.1); /* verde */

    &:hover {
        background-color: #218838; /* verde escuro */
        box-shadow: 0 4px 12px rgba(33, 136, 56, 0.2); /* verde escuro */
    }
}

div {
    justify-content: center;
}

.botaoConfirmaDelete {
    /* ... estilos existentes ... */
    background-color: rgba(0,0,0,0.7);
    color: #FFF;
}

.buttons-container {
    /* ... estilos existentes ... */
    padding: 20px;
}

h1 {
    justify-content: center;
    text-align: center;
    color: #333;
    font-weight: 600;
    
}

table {
    /* ... estilos existentes ... */
    box-shadow: 0 2px 8px rgba(0,0,0,0.1);
    width: 100%;
}

.table {
    width: 90%; /* Isso limita a tabela a 90% da largura do viewport, mas você pode ajustar conforme preferir */
    margin: 0 auto; /* Isso centraliza a div da tabela */
}

th, td {
    /* ... estilos existentes ... */
    font-weight: 400;
}

th {
    background-color: #28a745; /* verde */
    color: #FFF;
}

tr:nth-child(odd) {
    background-color: #e6f4ea; /* verde claro */
}

tr:hover {
    background-color: #d1ecd5; /* verde mais claro */
}

.container {
    /* ... estilos existentes ... */
    padding: 20px;
    box-shadow: 0 2px 8px rgba(0,0,0,0.1);
    background-color: #FFF;
}

form, .table, .botaoConfirmaDelete {
    display: inline-block;
    width: 100%;
    text-align: center;
}


h1, h2 {
    width: 100%;
}

</style>

